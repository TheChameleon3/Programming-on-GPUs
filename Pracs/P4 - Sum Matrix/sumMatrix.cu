...
// grid 2D block 2D
__global__ void sumMatrixOnGPU2D(float *A, float *B, float *C, int NXY)// int NX, int NY)
{
...
//dim3 block(dimx, dimy);
  //  dim3 grid(nxy/(dimx*dimy));
    ...
    sumMatrixOnGPU2D<<<grid, block>>>(d_MatA, d_MatB, d_MatC, nxy);//, ny);
